 ...
 */
const getAllNames = arr => arr.map(x => x.name);
 ...
 */
const getUsersByGender = (arr, gender) => arr.filter(x => x.gender === gender);
 ...
 */
const getInactiveUsers = arr => arr.filter(x => !x.isActive);
...
const getUsersByFriend = (arr, name) => 
    arr
    .filter(x => x.friends.includes(name))
...
const getAllUniqueSkills = arr => {
    let newArr = [];
    arr
    .reduce((acc, val) => acc.concat(val.skills), [])
    .sort()
    .filter(x => newArr.includes(x) ? newArr : newArr.push(x));
    return newArr;
}
