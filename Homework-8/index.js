...
titleImg.classList.add('fullview__img')

const previewImgList = document.createElement('ul');
previewImgList.classList.add('preview');
previewImgList.addEventListener('click', handleImgClick);


function createListItem({preview, fullview, alt}) {
    const listItem = document.createElement('li');
    listItem.classList.add('preview__item');

    const listImg = document.createElement('img');
    listImg.classList.add('preview__img');
    listImg.setAttribute('src', preview);
    listImg.setAttribute('data-fullview', fullview);
    listImg.setAttribute('alt', alt);

    listItem.append(listImg);
    previewImgList.append(listItem);
...
function creatList(arr) {
    ...
    arr.forEach(elem => {
        const el = createListItem(elem);
        items.push(el);
    }) 

    const titleImgSrc = items[0].firstChild.getAttribute('data-fullview');;
    titleImg.setAttribute('src', titleImgSrc);
    
    return items;
}


fullview.append(titleImg);
gallery.append(fullview, previewImgList);


function handleImgClick({target}) {
    if (target.nodeName !== 'IMG') return;
    titleImg.setAttribute('src', target.dataset.fullview);

    activateImg(target)
}

function activateImg(targetImg) {
    const listItemCollection = Array.from(previewImgList.children);

    listItemCollection.forEach(item => {
        const listItemCollectionImg = item.firstChild;

        if (listItemCollectionImg !== targetImg) {
            listItemCollectionImg.classList.remove('preview__img--active');
        } else {
            listItemCollectionImg.classList.add('preview__img--active');
        }        
    })
}

creatList(galleryItems);
  ...
  /*
    ...
    Создайте плагин галлереи используя ES6 класс. Добавьте поля и методы класса так, чтобы можно было создать любое количество галлерей на странице. Функционал плагина аналогичный заданию выше.
